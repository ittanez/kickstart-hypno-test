...
import { TestResultRequest, EmailResponse } from './types.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    const { email, score, category, description, senseDominant, timestamp }: TestResultRequest = await req.json();
    
    EmailLogger.logRequest({ email, score, category, senseDominant, timestamp });
